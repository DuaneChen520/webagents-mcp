...
| `--protocol <file>` | 在 prompt 末尾附加输出协议模板（模板随仓库分发，字节级一致） |
...
| `connect` | 桥不可达 / 扩展未连接 / 令牌不匹配 | `status` → 重启 IDE MCP 会话（桥由它拉起） |
| `queue` | 同站点排队超 4 分钟 | 稍候重试；前序任务是否卡死用 `inspect` 看 |
| `send` | 发送未生效（ack 丢失）或页面被人为操作 | `inspect <site>` 看现场（**禁 probe**） |
...
`textSource`（stream/dom-md/dom/none）· `mdVia`（cache/probe）· `mdLen`/`domLen`（长度快照）·
`streamId`/`streamStatus`（哪条流/什么终态）· `stage`（判定表段落）· `promptHash`（去重键）·
`runId` · `continued`/`repaired`（continue 与协议修复标记）。
...
两个站点的会话页状态（只读 inspect）→ 近 24h 失败模式统计。
核心项（令牌/桥/扩展）任一不过 → 退出码 1；站点页未打开、历史失败为警告级（⚠），不影响退出码。
...
- **MCP 与 CLI 是并列入口，CLI 不依赖 MCP**：桥连不上时 CLI 自拉桥（未驻留则为命令级，命令结束宿主可能回收）并等待扩展重连（`ensureExtensionReady`，默认上限 30s）后再执行。

...
- **要机器可读结果用 `--schema`**（配 `--protocol` 模板文件）：违规自动同会话修复一次，两次违规才报 `FAIL stage=quality`。
...
- `bridge stop` 只杀**自己驻留的桥**；外部桥（MCP 会话/用户终端拉的）需 `--force` 才会关闭，防误杀。
...
**禁止用 probe 取现场**（probe 会整页重载，摧毁验证弹窗与流记录）。
